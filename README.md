...
- **Problema:** a frota plug-in cresce ~2× mais rápido que a rede pública de recarga (a razão veículos/ponto público piorou de 17,9 para 19,6 em um semestre) e a recarga recai na garagem compartilhada — que chega a esse papel sem camada de gestão: rateio injusto, limite de potência e disputa por carregador.
- **Tese:** o gargalo não é o hardware (carregador AC é commodity), é a **gestão**.
- **Solução — EV ChargeOps:** cada recarga atribuída a uma unidade + rateio por kWh medido, justo e auditável + IA operacional sobre a telemetria (detecção de anomalias antes do fechamento da fatura, previsão de demanda). O modelo interno segue o vocabulário do OCPP, protocolo aberto, para que a integração com a GoodWe não dependa de um formato nosso.
- **Diferencial:** das cinco soluções de mercado analisadas, nenhuma oferece ao mesmo tempo rateio condominial nativo do Brasil, conta auditável ancorada na tarifa homologada e IA que segura a cobrança antes de fechar. A mais próxima, a NeoCharge, amarra a gestão ao próprio hardware; o EV ChargeOps nasce no carregador HCA G2 da GoodWe, sem exigir mudança nele.
...
**Opções de aprofundamento escolhidas: A (análise de mercado), B (pesquisa com usuários) e C (análise de dados públicos).** A Opção B aplicou o roteiro de [docs/entrevistas/roteiro.md](docs/entrevistas/roteiro.md) como survey estruturado (12 respondentes — moradores com e sem EV, um cético e um síndico); os insights estão incorporados ao dossiê, com leitura honesta de seus limites (amostra de conveniência, sinal qualitativo). A pesquisa **confirmou** os pilares da solução — rateio por consumo medido, fatura transparente e disponibilidade como prioridade — e expôs **tensões de comunicação e precificação** do modelo de cobrança, que a seção transforma em ajustes a considerar sem comprometer a viabilidade comercial.
...
Na análise de mercado (Opção A), mapeamos cinco soluções em quadrantes diferentes — Zaptec (hardware premium europeu), ChargePoint (plataforma global), NeoCharge (player nacional de condomínios), Copel EV (rede pública de concessionária) e Wallbox Pulsar Plus (carregador residencial de protocolo aberto, OCPP 1.6J, citado no enunciado do desafio) — com tabela comparativa de problema, funcionalidades, modelo de negócio e limitações. A leitura conjunta revela a lacuna que o EV ChargeOps ataca: **nenhuma das cinco oferece, ao mesmo tempo, rateio condominial nativo do contexto brasileiro, neutralidade de hardware via protocolo aberto e IA operacional sobre a telemetria.** O contraexemplo da Copel (eletropostos inativos sem sinalização, app mal avaliado) ainda nos deu um requisito de primeira classe: monitoramento de saúde dos pontos.

## Frente 2 — Base Regulatória e Técnica

Dossiê completo: [docs/frente-2-regulatorio.md](docs/frente-2-regulatorio.md) · Textos oficiais preservados: [docs/normas/](docs/normas/) · Acesso direto à plataforma SEMS+ (nível [O]): [docs/frente-2-sems-plus-acesso.md](docs/frente-2-sems-plus-acesso.md)

**Opções de aprofundamento escolhidas: A (mapeamento regulatório completo) e C (APIs complementares).** Sobre a Opção B (exploração da API), é preciso distinguir duas coisas sob o nome "API SEMS": a **OpenAPI de desenvolvedor** (credenciada, B2B) a GoodWe informou que **não disponibilizará** às equipes — essa via ficou inviabilizada e virou decisão arquitetural; mas em **26/06/2026 a GoodWe concedeu acesso de monitoramento à plataforma SEMS+** com a planta real da FIAP, e a equipe **explorou diretamente** a API web do carregador, documentando os endpoints reais (URL, método e status) e os campos de sessão exibidos na interface — o cerne do entregável da Opção B —, além do histórico real de sessões. Esse trabalho está registrado como evidência de primeira mão, nível **[O]** (ver "Estratégia de dados" e o anexo [docs/frente-2-sems-plus-acesso.md](docs/frente-2-sems-plus-acesso.md)); a parte fechada da API permanece coberta por levantamento documental.

O achado que define a arquitetura: **o carregador GoodWe HCA G2 não documenta OCPP (o único protocolo declarado é Modbus TCP) e a API SEMS é fechada — e a documentação oficial dela sequer menciona carregadores.** Lemos na íntegra os três documentos oficiais do equipamento (dois datasheets e o manual G2 de 69 páginas) e os dois documentos públicos da API; o contrato real usado pela comunidade vem de engenharia reversa do app SEMS+. Com o acesso direto à plataforma SEMS+ concedido em 26/06/2026 (planta real da FIAP), refinamos o diagnóstico: as sessões encerradas e o campo de cartão **existem** na API web — mas é API privada não-contratual, o RFID chegou em auto-start (sem vínculo a pessoa) e não há exportação estruturada de recarga. Conclusão de engenharia: ancorar a plataforma nessa API seria construir sobre areia; a resposta é uma camada de ingestão plugável que normaliza qualquer fonte para um modelo interno OCPP-compatível. Documentamos também o que cada interface do HCA G2 permite (RFID como chave de atribuição de sessão, LAN/Wi-Fi para telemetria, RS-485 para medidor MID certificado, Bluetooth para comissionamento) e o achado positivo: a medição certificada via RS-485 é a fonte de kWh com lastro metrológico que um rateio contestável em assembleia exige.
...
A GoodWe comunicou que **não disponibilizará a OpenAPI de desenvolvedor (credenciada) do SEMS** para as equipes do desafio — embora tenha concedido, em 26/06/2026, **acesso de monitoramento à plataforma SEMS+** com a planta real da FIAP. Tratamos a indisponibilidade da API credenciada não como obstáculo a contornar, mas como decisão arquitetural de primeira classe — até porque nosso levantamento, e depois o acesso direto à plataforma, mostrou que mesmo com acesso o contrato seria frágil: API privada não-contratual, sem documentação oficial para carregadores; o acesso direto confirmou que, embora as sessões encerradas e o campo de cartão existam na API web, o RFID chega em auto-start (sem vínculo a pessoa) e não há exportação estruturada de recarga. A estratégia é um tripé:
...
Honestidade declarada: **nenhum evento real de um HCA G2 atravessou este pipeline até aqui.** As 18 sessões reais do HCA G2 da FIAP que observamos no SEMS+ (Frente 2, nível [O]) foram *vistas* na plataforma, não *ingeridas* pelo pipeline — e são da planta de demonstração do laboratório, não do condomínio-alvo. Dado real do carregador do campus só será possível na visita ao Energy Innovation Lab, na Sprint 2, se viável; a arquitetura foi desenhada para absorvê-lo pela camada de ingestão, sem alteração estrutural (justificativa técnica na Frente 3).
